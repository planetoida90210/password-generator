...
const PasswordCopyboard = ({showCopied, setShowCopied, setGeneratedPassword, generatedPassword}) => {
  ...
  return (
    <div className="flex w-[343px] h-[64px] text-center md:w-[540px] md:h-[80px] bg-[#24232C] mt-4 md:mt-8 rounded-md">
        ...
        />
        <button 
        className="pr-[16px] md:pr-[32px] xl:pr-[36px] text-[#A4FFAF] cursor-pointer"
        onClick={() => navigator.clipboard.writeText(generatedPassword).then(() => {setShowCopied(true), setGeneratedPassword('')})}
        >
            { showCopied
             ? (
